...
- ResultSetMetaData meta = rs.getMetaData();
  - **getColumnName**(int column)：获取指定列的名称(**获取表的列名**) 不推荐使用
  - **getColumnLabel**(int column)：**获取指定列的别名 (列的别名)，如果没有取别名，默认取得是列的名称，针对java的属性名与数据库字段名不一样时**
...
*关于增删改可以调用preparedStatement.executeUpdate()来查看操作的行数*
 ...
 * 修改3：使用更新的mysql 驱动：mysql-connector-java-5.1.37-bin.jar 5.17版本不支持batch操作
 * 
 */
@Test
...
`增删改，默认是每次提交一次就提交一次,delete 默认是这样，但是可以更改，truncate清空表是自动提交的`

```java
/*
* 层次四：在层次三的基础上操作 delete可以实现回滚操作
...
![image-20220301185717812](G:\development\3_JDBC\尚硅谷—宋红康-JDBC\JDBC\1-课件\课件-md\尚硅谷_宋红康_JDBC.assets\image-20220301185717812.png)
...
2. **一致性（Consistency）**
    事务必须使数据库从一个一致性状态变换到另外一个一致性状态。**我加了100块，你减了100块，我们总和还是不变的，一致性不变化。**
...
![image-20220302111135490](G:\development\3_JDBC\尚硅谷—宋红康-JDBC\JDBC\1-课件\课件-md\尚硅谷_宋红康_JDBC.assets\image-20220302111135490.png)
