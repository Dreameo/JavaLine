...
## MyBatis特性
1. MyBatis 是支持定制化 SQL（自己写）、存储过程以及高级映射(普通映射为数据库表里的字段名与java对象属性名要一样默认映射)的优秀的持久层框架
2. MyBatis 避免了几乎所有的 JDBC 代码和手动设置参数以及获取结果集
3. MyBatis可以使用简单的XML或注解用于配置和原始映射，将接口和Java的POJO（Plain Old Java Objects，普通的Java对象）映射成数据库中的记录
4. MyBatis 是一个 半自动的ORM（Object Relation Mapping）框架 （SQL需要我们自己写，所以叫做半自动，jdbc是手动，ORM实体类对象与数据库表）
## MyBatis下载
...
- MyBatis
	- 轻量级，性能出色  
	- SQL 和 Java 编码分开（两个配置文件，一个核心配置文件，一个映射文件），功能边界清晰。Java代码专注业务、SQL语句专注数据  
...
1. 加入依赖
	...
	```xml
	<!-- log4j日志 从log4j 1.2.17之后jar改名为log4j-core,上面会报错，降版本解决，但是用下面core还是不行-->
	<dependency>
...
<configuration>
   ...
   jdbc.properties文件，最好用功能前缀进行表示，防止核心配置文件引入多个properties文件，键名冲突，比如使用jdbc.driver jdbc.url
   -->
    ...
    <typeAliases>
        <!-- 查询条件中，如果每次都要写全包名，很麻烦，使用别名，类型别名不区分大小写
        typeAlias：设置某个具体的类型的别名
        属性：
        type：需要设置别名的类型的全类名
        alias：设置此类型的别名，且别名不区分大小写。若不设置此属性，该类型拥有默认的别名，即类名，且不区分大小写，但如果写了alia那么必须以alias为准
        -->
    ...
    <environments default="mysql_test">
        ...
        <environment id="mysql_test">
            ...
            属性：
	            ...
	            type="MANAGED"：设置事务被管理，例如spring中的AOP，Spring声明式事务管理
            -->
            <transactionManager type="JDBC"/>
            <!--
            dataSource：设置数据源
...
- ${}使用字符串拼接的方式拼接sql，若为**字符串类型或日期类型**的字段进行赋值时，**需要手动加单引号**；但是#{}使用占位符赋值的方式拼接sql，此时为字符串类型或日期类型的字段进行赋值时，可以自动添加单引号
## 单个字面量类型的参数
 ...
 * 根据用户id查询用户信息为map集合  
 * @param id  
 * @return  没有一个实体类与数据库表对应，就会转化为map，也可能查询多张表的时候用
 * 以字段为键，字段的值为值 
 */  
...
```java
/**
一张表很多数据，如果数据很多，我们mysql直接查很影响性能，我们可以将表水平切分为多个表格，共同存储这些数据，表不固定
 * 查询指定表中的数据，只能使用$号
 * @param tableName 
 * @return java.util.List<com.atguigu.mybatis.pojo.User>
...
```
## 添加功能获取自增的主键
- 使用场景 1：M, ,不管是一对多还是多对一，表关系都要设置在多的一方。
...
- 若字段名和实体类中的属性名不一致，则可以通过**resultMap设置自定义映射**，即使字段名和属性名一致的属性也要映射，也就是全部属性都要列出来
```xml
...
- 在where后面添加一个恒成立条件`1=1`
	- 这个恒成立条件并不会影响查询的结果
	- 这个`1=1`可以用来拼接`and` （**因为xml文件中，&是特殊符号**）语句，例如：当empName为null时
...
> （查询的数据存起来，下次再查的时候直接从缓存中查找，从而查找速度更加快速）

## MyBatis的一级缓存 （默认开启的）
- 一级缓存是SqlSession级别的，通过同一个SqlSession查询的数据会被缓存，下次查询相同的数据，就会从缓存中直接获取，不会从数据库重新访问  
- 使一级缓存失效的四种情况：  (缓存只对查询有效)
...
> 一个SqlSessionFactory可以获取多个SqlSession
...
- 二级缓存开启的条件
	...
	2. 在映射文件中设置标签<cache />
	3. 二级缓存必须在SqlSession关闭或提交之后有效（commit,close）
	4. 查询的数据所转换的实体类类型必须实现序列化的接口
- 使二级缓存失效的情况：两次查询之间执行了任意的增删改，会使一级和二级缓存同时失效
## 二级缓存的相关配置
- 在mapper配置文件中添加的cache标签可以设置一些属性
- eviction属性：缓存回收策略  （缓存数据存在内存中，所以不能无限制缓存）
...
| ehcache | EHCache核心包 |
| slf4j-api | SLF4J日志门面包提供接口，日志的实现给其他厂商提供就像jdbc一样 |
...
<generatorConfiguration>
    ...
    MyBatis3Simple: 生成基本的CRUD（清新简洁版），增删改，查询所有，根据id查询
